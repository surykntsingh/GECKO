...
class BClassifier(nn.Module):
    def __init__(self, input_size, input_size_deep, stop_gradient='no', top_k=10, mlp_layers=4): # K, L, N
        ...
        self.mlp_mixer = MLPMixer(num_tokens=input_size, dim=top_k, hidden_dim=256 #=input_size_deep
    ...
    def forward(self, feats, feats_deep, training='no'): # B x N x K, B x N x C
        ...
        return B.sum(1), perturbed_topk_feats.sum(1), B_deep_proj, B_deep, A_feat, A_patch
...
class MILNet(nn.Module):
    ...
    def forward(self, x, x_deep, training='no'):
        B, perturbed_topk_feats, B_deep_proj, B_deep, A_feat, A_patch = self.b_classifier(x, x_deep, training=training)
        return B, perturbed_topk_feats, B_deep_proj, B_deep, A_feat, A_patch
